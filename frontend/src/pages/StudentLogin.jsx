...
import { GraduationCap, Mail, Lock, AlertCircle, KeyRound, CheckCircle } from 'lucide-react';

export default function StudentLogin({ onSuccess, onSwitchToSignUp, onBackToRole }) {
  const { login } = useAuth();
    ...
    <div className="min-h-screen bg-[#07090e] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="glass-panel-glow p-8 rounded-3xl max-w-md w-full relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-extrabold text-white">Student Login</h2>
          <p className="text-xs text-slate-400">Sign in to your Student AI Interview Workspace</p>
        </div>
      ...
      {showForgotModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-panel p-6 rounded-3xl max-w-sm w-full space-y-4 border border-purple-500/40 relative">
            <div className="flex items-center space-x-3">
              <KeyRound className="w-6 h-6 text-purple-400" />
              <h3 className="text-lg font-bold text-white">Reset Password</h3>
            </div>
            <p className="text-xs text-slate-300">Enter your email address to receive password recovery instructions.</p>
